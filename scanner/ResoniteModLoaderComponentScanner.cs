...
public sealed class ResoniteApiCatalogScannerMod : ResoniteMod
{
    ...
    private const string ScannerVersion = "1.11.1";
    ...
    private static bool IsCatalogRelevantType(
        ...
        Type? workerBase)
    {
        // Builder-generated adapters call this versioned infrastructure directly.
        // Do not duplicate the shared runtime as hundreds of user-facing catalog nodes.
        if (type.Assembly == typeof(ResoniteMod).Assembly &&
            string.Equals(type.Namespace, "ResoniteModLoader.BuilderRuntime", StringComparison.Ordinal))
        {
            return false;
        }
                ...
                string health =
                    JsonSerializer.Serialize(
                        new
                        {
                            ...
                            builderRuntimeContractVersion =
                                CurrentBuilderRuntimeContractVersion(),
                            runtimeBridgeVersion =
                                RuntimeBridgeVersion,
                            runtimeBridgeReady =
                                true,
    ...
    private static int CurrentBuilderRuntimeContractVersion()
    {
        try
        {
            Type? runtime = typeof(ResoniteMod).Assembly.GetType(
                "ResoniteModLoader.BuilderRuntime.GraphRuntimeV1", false, false);
            return runtime?.GetField("ContractVersion", BindingFlags.Public | BindingFlags.Static)
                ?.GetRawConstantValue() is int version ? version : 0;
        }
        catch
        {
            return 0;
