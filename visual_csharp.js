(() => {
  ...
  registerNode("csharp.file", {
    ...
    description: "A reusable C# source container. Its complete source stays in this Runtime Graph node's Actions. Convert to Node Graph creates or refreshes the separate editable syntax graph.",
    parameters: [
      ...
      code("source", "C# 14 source", "", "Complete source of this custom file. Convert to Node Graph parses this field with bundled .NET 10 Roslyn. When the syntax graph changes, its generated code is written back here.", 22)
    ],
    inputs: [],
  ...
  function storeCustomCSharpFragment(fragment, host, state, originalSource = null) {
    ...
    delete state.customCSharpFiles[runtimeFileNode.id];
  ...
  function installCSharpImportControl() {
    ...
    input.addEventListener("change", async () => {
      ...
      try {
        ...
        pendingText.textContent =
          `${file.name} passed Roslyn C# 14 syntax validation. One Custom C# File node will be added to the Runtime Graph with the complete file text in its Actions. About ${syntaxItemCount.toLocaleString()} AST, token and trivia elements are created only when you later choose Convert to Node Graph. Nothing has been imported yet.`;
