(() => {
  ...
  registerNode("csharp.file", {
    ...
    group: GROUPS.project,
    symbol: ".CS",
    description: "Emits one source file from connected visual C# syntax. No raw source block is stored.",
    parameters: [
  ...
  registerCodegenPlugin({
    ...
    collect(api) {
      ...
      for (const node of fileNodes) {
        ...
        const connection = incoming.get(`${node.id}:content`);
        if (!connection) {
          api.diagnostic(`C# File '${fileName}' has no connected Compilation unit.`);
          continue;
        }
        const body = renderNode(connection.fromNode);
