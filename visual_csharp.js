(() => {
  ...
  const customCSharpModeParameter = () => ({
    ...select(
      ...
      [
        ...
        { value: "runtimeMember", label: "Runtime Graph Member" },
        { value: "mainMember", label: "Main Mod Member" }
      ],
      "file",
      "Changes what this one Custom C# node emits. Only File mode owns an editable visual C# file graph."
    ),
    affectsPorts: true,
    affectsNode: true,
  ...
  const editorColor = (
    ...
    default: defaultValue,
    help:
      "Editor-only appearance. Uses the Builder JavaScript color picker and is never inserted into generated C# source.",
  ...
  function customCSharpRuntimeDefinition(node) {
    ...
    if (mode === "action") {
      return {
        ...base,
        description:
          "Emits a custom C# action with independently typed value inputs.",
        parameters: [
          customCSharpModeParameter(),
          ...customCSharpEditorParameters(),
          number(
    ...
    if (mode === "expression") {
      return {
        ...base,
        description:
          "Emits a custom C# expression with independently typed value inputs.",
        parameters: [
          customCSharpModeParameter(),
          ...customCSharpEditorParameters(),
          number(
    ...
    return {
      ...
      description: mainMember
        ? "Emits a custom member into the main generated mod class."
        : "Emits a custom member into the generated Runtime Graph class.",
      parameters: [
        customCSharpModeParameter(),
        ...customCSharpEditorParameters(),
        code(
  ...
  function createRoslynImportFragment(source, parseResult, options = {}) {
    ...
    const tryAddCompactUsing = (syntaxNode, depth) => {
      ...
      return addNode("csharp.usingDirective", parameters, depth, "Using Directive");
    };
  ...
  function installCSharpImportControl() {
    ...
    const setImportButtonAvailability =
      available => {
        const reason =
          "Confirm the C# 14/Roslyn warning before choosing a source file.";
        const shared =
    ...
    button.addEventListener("click", () => {
      if (!acknowledgement.checked) {
        localStatus.textContent =
          "Confirm the C# 14/Roslyn warning first.";
