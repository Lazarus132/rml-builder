(() => {
  ...
  function updateNarrationOutlines() {
    for (const group of narrationOutlineGroups) {
      ...
      group.element.dataset.rmlBoxLeft =
        String(rect.left);
      group.element.dataset.rmlBoxTop =
        String(rect.top);
      group.element.dataset.rmlBoxWidth =
        String(rect.right - rect.left);
      group.element.dataset.rmlBoxHeight =
        String(rect.bottom - rect.top);
    }
  }
      ...
      if (compactMenuHandoff) {
        ...
        const resumed = tourDebugAssert(
          ...
          compactMenuHandoff?.passed === true &&
            ...
            Boolean(outlineRect) &&
            !outline?.element?.classList.contains(
              "rml-setup-hidden"
            ),
          {
            segmentIndex,
  ...
  function measureHiddenGraphInspectorRevealRect() {
    ...
    const wasMeasurementHidden =
      panel.classList.contains(
        "rml-setup-measure-hidden"
      );
  ...
  function placeCardOutsideSemanticScene() {
    ...
    card.dataset.rmlCardLeft =
      String(placement.left);
    card.dataset.rmlCardTop =
      String(placement.top);
    window.RMLClassStyles?.sync(card);
  ...
  function positionShades(target, options = {}) {
    ...
    const setShade = (
      ...
      if (!visible) return;
      shade.dataset.rmlBoxLeft = String(left);
      shade.dataset.rmlBoxTop = String(top);
      shade.dataset.rmlBoxWidth = String(width);
      shade.dataset.rmlBoxHeight = String(height);
    };
    const viewport = tourViewport();
    const gap = 9;
    ...
    setShade(
      ...
      bottomY,
      viewport.width,
      Math.max(0, viewport.bottom - bottomY)
    );
    setShade(
      "left",
      true,
    ...
    setShade(
      ...
      right,
      topY,
      Math.max(0, viewport.right - right),
      Math.max(0, bottomY - topY)
    );
  }
  ...
  function fitNarrationCardToContent({ reset = false, followText = true } = {}) {
    ...
    if (reset) {
      ...
      delete card.dataset.rmlAdaptiveCardHeight;
      delete card.dataset.rmlAdaptiveTextMinHeight;
    }
    ...
    const applyNarrationWidth = () => {
      card.dataset.rmlAdaptiveCardWidth =
        String(Math.round(requestedWidth));
      card.dataset.rmlAdaptiveCardMaxWidth =
    ...
    card.dataset.rmlAdaptiveTextMinHeight =
      String(minimumTextHeight);

    const previousScrollTop = text.scrollTop;
    const measureNaturalCard = () => {
      card.classList.add(
        "rml-setup-natural-card-measure"
      );
      window.RMLClassStyles?.sync(card);
    ...
    card.dataset.rmlAdaptiveCardMaxHeight =
      String(Math.round(maximumHeight));
    card.dataset.rmlAdaptiveCardHeight =
      String(Math.ceil(requestedHeight));
    window.RMLClassStyles?.sync(card);
    void card.offsetHeight;
    ...
    modalSurfaceState = {
      ...
      mountedClass:
        dialog.classList.contains(
          "rml-setup-modal-host"
        ),
      top: dialog.dataset.rmlModalTop,
      left: dialog.dataset.rmlModalLeft
    };

    dialog.classList.add(
      "rml-setup-modal-host"
    );
    dialog.dataset.rmlModalTop =
      String(rectangle.top);
    dialog.dataset.rmlModalLeft =
      String(rectangle.left);
    window.RMLClassStyles?.sync(dialog);
  ...
  async function stabilizeTeacherMouseAboveDialogTarget(
    ...
    for (
      ...
      mouse.dataset.rmlMouseDuration = "0";
      window.RMLClassStyles?.sync(mouse);
  ...
  function restoreTourSurfaceFromModal(closeDialog = false) {
    ...
    if (state.top === undefined) {
      delete state.dialog.dataset.rmlModalTop;
    } else {
      state.dialog.dataset.rmlModalTop = state.top;
    }
    if (state.left === undefined) {
      delete state.dialog.dataset.rmlModalLeft;
    } else {
      state.dialog.dataset.rmlModalLeft = state.left;
    }
  ...
  function teacherMouseCoordinates() {
    ...
    const x = Number.parseFloat(
      mouse.dataset.rmlMouseX
    );
    const y = Number.parseFloat(
      mouse.dataset.rmlMouseY
    );
  ...
  function positionLiveControlsAwayFromMouseRoute(
    ...
    controls.classList.add(
      "rml-setup-live-controls-positioned"
    );
    controls.dataset.rmlControlsLeft =
      String(selected.rect.left);
    controls.dataset.rmlControlsTop =
      String(selected.rect.top);
  ...
  function setTeacherMousePoint(
    ...
    mouse.dataset.rmlMouseDuration =
      String(Math.max(0, duration));
    mouse.dataset.rmlMouseX =
      String(point.x);
    mouse.dataset.rmlMouseY =
      String(point.y);
  ...
  async function moveMouse(point, duration = 720, runId = demoRunId) {
    ...
    if (ghost && !ghost.hidden) {
      ghost.classList.add(
        "rml-setup-drag-ghost-animated"
      );
      ghost.dataset.rmlMoveDuration =
        String(effectiveDuration);
      ghost.dataset.rmlBoxLeft =
        String(point.x + 16);
      ghost.dataset.rmlBoxTop =
        String(point.y + 12);
    }
  ...
  function showFaithfulDragGhost(source, point) {
    ...
    dragGhost.dataset.rmlBoxLeft =
      String(point.x + 16);
    dragGhost.dataset.rmlBoxTop =
      String(point.y + 12);
  }
  ...
  function pulseAt(element, className = "rml-setup-demo-pulse") {
    ...
    pulse.dataset.rmlBoxLeft =
      String(rect.left);
    pulse.dataset.rmlBoxTop =
      String(rect.top);
    pulse.dataset.rmlBoxWidth =
      String(rect.width);
    pulse.dataset.rmlBoxHeight =
      String(rect.height);
  ...
  function showDemoLabel(text, point, avoidTarget = null) {
    ...
    demoLabel.dataset.rmlBoxLeft =
      String(best.left);
    demoLabel.dataset.rmlBoxTop =
      String(best.top);
  }
  ...
  function showKeys(keys, point = null) {
    ...
    holder.dataset.rmlBoxLeft =
      String(Math.min(
        viewport.right - 120,
        Math.max(viewport.left + 12, location.x + 34)
      ));

    holder.dataset.rmlBoxTop =
      String(Math.min(
        viewport.bottom - 60,
  ...
  function setRealPortGlow(socket, active) {
    ...
    if (stepPhase === "demonstrating") {
      ...
      if (actionRow) {
        delete actionRow.dataset.rmlSetupPortGlow;
      }
      return;
    }
    ...
    if (active) {
      ...
      delete row.dataset.rmlSetupPortGlow;
    }
  }
  ...
  async function animateGraphPortTravel(
    ...
    travel.dataset.rmlTravelColor =
      snapshot.color;
    travel.dataset.rmlTravelDuration =
      String(duration);
    travel.dataset.rmlBoxLeft =
      String(snapshot.point.x);
    travel.dataset.rmlBoxTop =
      String(snapshot.point.y);
  ...
  async function runGraphPortFlipDemo(runId) {
    ...
    applyGraphConnectionScene(nodeId, {
      highlightCounterparts: false
    });
  ...
  async function setGraphModeForPreparation(wantsGraph, runId) {
    if (wantsGraph) {
      try {
        ...
        console.warn(
          "The guided tour could not prepare the Runtime Graph view assets.",
          error
        );
        return false;
      }
  ...
  function restoreControlledRepeatDialogPresentation(index, phase) {
    ...
    ui.progress.dataset.rmlProgress =
