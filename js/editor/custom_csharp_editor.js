(() => {
  ...
  const STYLE_TEXT = `
    :root { color-scheme: dark; --editor-font-size: 16px; --editor-line-height: 24px; --rml-workbench-background: attr(data-rml-workbench-background type(<color>), #181818); --rml-code-background: attr(data-rml-code-background type(<color>), #000000); --rml-gutter-background: attr(data-rml-gutter-background type(<color>), #000000); --rml-panel-background: attr(data-rml-panel-background type(<color>), #181818); --rml-overlay-background: attr(data-rml-overlay-background type(<color>), #252526); --rml-status-background: attr(data-rml-status-background type(<color>), #68217a); --rml-selection-background: attr(data-rml-selection-background type(<color>), #264f78); --rml-code-text: attr(data-rml-code-text type(<color>), #ffffff); --rml-ui-text: attr(data-rml-ui-text type(<color>), #cccccc); --rml-gutter-text: attr(data-rml-gutter-text type(<color>), #858585); --rml-status-text: attr(data-rml-status-text type(<color>), #ffffff); --rml-accent: attr(data-rml-accent type(<color>), #b789ff); --rml-caret: attr(data-rml-caret type(<color>), #ffffff); --line: #45414f; --line-strong: #665d75; --panel-deep: #121019; --text: #e8e3ef; --muted: #9790a2; --accent-dark: #a476ff; --accent-soft: rgba(164, 118, 255, .16); }
    * { box-sizing: border-box; }
    ...
    .rml-graph-searchable-popup { position: fixed; z-index: 100000; display: grid; min-width: 180px; max-width: attr(data-rml-popup-max-width px, 560px); width: attr(data-rml-popup-width px, 180px); left: attr(data-rml-popup-left px, 0px); top: attr(data-rml-popup-top px, 0px); gap: 6px; padding: 7px; border: 1px solid var(--line-strong); border-radius: 9px; background: linear-gradient(180deg, rgba(24, 22, 34, .995), rgba(13, 12, 19, .995)); box-shadow: 0 18px 48px rgba(0, 0, 0, .58), inset 0 1px rgba(255, 255, 255, .035); }
    ...
    .editor-shell { position: relative; display: grid; grid-template-columns: auto minmax(0, 1fr); min-width: 0; min-height: 0; overflow: hidden; background: var(--rml-code-background); }
    .line-gutter { min-width: 58px; min-height: 0; overflow: hidden; border-right: 1px solid #292929; background: var(--rml-gutter-background); color: var(--rml-gutter-text); -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; }
    .line-gutter pre { margin: 0; padding: 14px 10px 14px 6px; font: var(--editor-font-size)/var(--editor-line-height) ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; text-align: right; white-space: pre; will-change: transform; transform: translateY(attr(data-rml-scroll-offset px, 0px)); }
    .editor-content { position: relative; min-width: 0; min-height: 0; overflow: hidden; }
    textarea { position: relative; z-index: 1; width: 100%; height: 100%; min-width: 0; min-height: 0; resize: none; border: 0; border-radius: 0; padding: 14px 18px; outline: none; background: transparent; color: var(--rml-code-text); caret-color: var(--rml-caret); font: var(--editor-font-size)/var(--editor-line-height) ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; tab-size: 4; white-space: pre; overflow: auto; -webkit-overflow-scrolling: touch; scrollbar-width: auto; scrollbar-color: var(--rml-accent) var(--rml-code-background); }
    ...
    .editor-shell.node-drop-active { box-shadow: inset 0 0 0 2px var(--rml-accent), inset 0 0 34px rgba(183, 137, 255, .20); box-shadow: inset 0 0 0 2px var(--rml-accent), inset 0 0 34px color-mix(in srgb, var(--rml-accent) 20%, transparent); }
    ...
    .settings-color-line { display: block; height: 4px; border: 1px solid rgba(255, 255, 255, .32); border-radius: 999px; background-color: attr(data-rml-color type(<color>), transparent); box-shadow: 0 0 0 1px rgba(0, 0, 0, .42); pointer-events: none; }
  ...
  function mount(options = {}) {
    ...
    const createPresentationDropdown = () => {
      ...
      const positionMenu = () => {
        ...
        menu.dataset.rmlPopupWidth =
          String(Math.round(width));
        menu.dataset.rmlPopupMaxWidth =
          String(
            Math.max(
              160,
        ...
        window.RMLClassStyles?.sync(menu);
        ...
        const top = measuredHeight > below && above > below
          ...
          : rect.bottom + gap;
        menu.dataset.rmlPopupLeft =
          String(Math.round(left));
        menu.dataset.rmlPopupTop =
          String(
            Math.round(
              Math.max(
                viewportTop + margin,
