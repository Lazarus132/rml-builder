...
function ensureLazyStyleBundle(name) {
  const loader = window.RMLStyleLoader;
  if (!loader?.ensure) {
    return Promise.reject(
      new Error("The deferred stylesheet loader is unavailable.")
    );
  }
  return loader.ensure(name);
}

function ensureLazyScriptBundle(name) {
  const loader = window.RMLScriptLoader;
  if (!loader?.ensure) {
    return Promise.reject(
      new Error("The deferred JavaScript module loader is unavailable.")
    );
  }
  return loader.ensure(name);
}

function installLazyStyleWarmup(element, name) {
  if (!element || element.dataset.rmlStyleWarmup === name) {
    return;
  }
  element.dataset.rmlStyleWarmup = name;
  const warm = () => {
    void ensureLazyStyleBundle(name).catch(error => {
      console.warn(`${name} styles could not be prepared early.`, error);
    });
  };
  ...
  colorPickerAdaptiveFitLoadPromise =
    ensureLazyStyleBundle("preview")
      .then(() => new Promise((resolve, reject) => {
        ...
        script.src =
          new URL(
            "../ui/colorpicker_adaptive_fit.js?v=3-class-driven-styles-v751",
            APP_SCRIPT_BASE_URL
          ).href;
...
function applyInlineRowPreviewLayout(
  ...
  element.dataset.rmlInlineWidthPercent =
    inlineRowWidthText(
...
function getTypedNodeGraphContribution() {
  ...
  if (
    ...
    return {
      active: true,
      diagnostics: [
        "The typed node graph generator module is not loaded."
      ],
      warnings: [],
      files: [],
      applyStatements: {},
...
function exampleProjectUrl() {
  return new URL(
    `../../assets/data/${EXAMPLE_PROJECT_FILE_NAME}`,
    APP_SCRIPT_BASE_URL
  );
}
...
function moveOptionPointerGhost(
  ...
  optionPointerGhost.dataset.rmlGhostX =
    String(clientX + 14);
  optionPointerGhost.dataset.rmlGhostY =
    String(clientY + 14);
}
...
function moveNodePointerGhost(
  ...
  nodePointerGhost.dataset.rmlGhostX =
    String(clientX + 14);
  nodePointerGhost.dataset.rmlGhostY =
    String(clientY + 14);
}
...
function movePalettePointerGhost(
  ...
  palettePointerGhost.dataset.rmlGhostX =
    String(clientX + 14);
  palettePointerGhost.dataset.rmlGhostY =
    String(clientY + 14);
}
  ...
  const dragHitTestElement =
    event => {
      ...
      for (
        ...
        previous.element.classList.toggle(
          "rml-hit-test-transparent",
          previous.ignored
        );
      }
    ...
    <div
      ...
      <div class="custom-color-picker-body">
        ...
        <label class="custom-color-slider-control alpha-control">
          ...
          <input
            ...
            value="${alphaByte}"
            data-color-alpha
            data-rml-alpha-color="${escapeHtml(preview.hex)}"
            aria-label="Alpha">
        </label>
      ...
      <div class="custom-color-picker-actions">
        <div
          ...
          data-color-result
          data-rml-result-color="${escapeHtml(preview.cssColor)}"
          data-rml-result-glow="${escapeHtml(preview.cssColor)}"
          data-rml-result-glow-size="${glowSize}">
...
function updateCustomColorPicker(
  ...
  inlinePicker.dataset.colorStrength =
    String(strength);
  inlinePicker.dataset.rmlPreviewColor =
    visualCss;
  inlinePicker.dataset.rmlPreviewGlow =
    visualCss;
  inlinePicker.dataset.rmlPreviewGlowSize =
    String(glowSize);
  ...
  if (alphaInput) {
    ...
    alphaInput.dataset.rmlAlphaColor =
      hex;
  }
  if (alphaOutput) {
  ...
  if (result) {
    result.dataset.rmlResultColor =
      visualCss;
    result.dataset.rmlResultGlow =
      visualCss;
    result.dataset.rmlResultGlowSize =
      String(glowSize);
  }
...
function ensureUniversalCustomSelect(select) {
  ...
  const positionPopup = () => {
    ...
    popup.dataset.rmlPopupMaxWidth =
      String(
        Math.max(
          160,
...
function handleSettingsPreviewInput(event) {
  ...
  if (target.matches("[data-preview-range]")) {
    ...
    target.dataset.rmlRangeProgress =
      `${settingsPreviewRangeProgress(
        Number(target.value),
        Number(target.min),
        Number(target.max)
...
async function openSettingsPreview() {
  ...
  try {
    await ensureLazyStyleBundle("preview");
    await ensureColorPickerAdaptiveFitLoaded();
  } catch (error) {
    console.error("Preview assets could not be loaded.", error);
    void showBuilderNotice({
      ...
      message:
        error instanceof Error
          ...
          : String(error),
      details:
        "The Configuration Outline and generated project remain unchanged.",
      confirmLabel: "OK"
    });
    return;
  }

  if (sequence !== settingsPreviewOpenSequence) {
    return;
  }
...
async function openProjectDialog() {
  ...
  try {
    ...
    console.error("Project dialog styles could not be loaded.", error);
    void showBuilderNotice({
      tone: "warning",
      kicker: "Project manager unavailable",
      title: "The project dialog could not be opened",
      message:
        error instanceof Error
          ? error.message
          : String(error),
      details:
        "The open project and its local draft remain unchanged.",
      confirmLabel: "OK"
    });
    return;
  }
...
function setInformationNodeScope(
  ...
  if (targetScope === "runtime") {
    if (
      typeof window.RMLModNodeRegistry
        ?.getNodeDefinitions !==
        "function"
    ) {
      void ensureLazyScriptBundle(
        "node-library"
      )
        .then(() => {
          if (
            elements.informationDialog
              ?.dataset.informationNodeScope ===
              "runtime"
          ) {
            renderInformationNodeReference();
          }
        ...
        .catch(error => {
          console.error(
            "The Runtime Graph node reference could not be loaded.",
            error
          );
        });
    }
...
function ensureSetupAssistantLoaded(firstRun = false) {
  ...
  if (window.RMLBuilderSetupAssistant?.start) {
    return ensureLazyStyleBundle("setup")
      .then(() => {
        window.RMLBuilderSetupAssistant.start({ firstRun });
        return true;
      })
      .catch(error => {
        console.warn("Guided tour styles could not be loaded.", error);
        return false;
      });
  }
  ...
  setupAssistantLoadPromise =
    ensureLazyStyleBundle("setup")
      .then(() => new Promise((resolve, reject) => {
    ...
    value: Object.freeze({
      ...
      stepHeldOptionHorizontal(
        ...
        const markerBeforeFeedback =
          optionDragFeedbackPlaceholder;
        const anchoredTop =
          markerBeforeFeedback?.dataset.setupOptionAnchorTop ||
          "";
        const anchoredHeight =
          markerBeforeFeedback?.dataset.setupOptionAnchorHeight ||
          "";
        ...
        if (marker?.isConnected) {
          if (anchoredTop) {
            marker.dataset.setupOptionAnchorTop =
              anchoredTop;
            marker.dataset.rmlPlaceholderTop =
              anchoredTop;
          }
          if (anchoredHeight) {
            marker.dataset.setupOptionAnchorHeight =
              anchoredHeight;
            marker.dataset.rmlPlaceholderHeight =
              anchoredHeight;
          }
        }
        requestDragPlaceholderVisibility();
...
async function initialize() {
  ...
  installLazyStyleWarmup(
    elements.settingsPreviewOpen,
    "preview"
  );
  installLazyStyleWarmup(
    elements.informationOpen,
    "information"
  );
  installLazyStyleWarmup(
  ...
  "RMLDynamicOutlineHost",
  {
    value: Object.freeze({
      ...
      commit() {
        ...
        try { window.renderBuilderCanvas?.(); } catch {}
