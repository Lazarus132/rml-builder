(() => {
  ...
  if (typeof window.RMLScheduleTask !== "function") {
    ...
    const flush = () => {
      pending = false;
      const batch = taskQueue.splice(0);
      for (const callback of batch) {
        try { callback(); } catch (error) { queueMicrotask(() => { throw error; }); }
      }
      if (taskQueue.length) scheduleFlush();
    };
    const scheduleFlush = () => {
      if (pending) return;
      pending = true;
      if (globalThis.scheduler?.postTask) {
        globalThis.scheduler.postTask(flush, { priority: "user-visible" }).catch(error => { pending = false; queueMicrotask(() => { throw error; }); });
      } else if (channel) {
    ...
    if (channel) channel.port1.onmessage = flush;
