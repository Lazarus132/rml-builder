(() => {
  ...
  const __rmlWorkerYieldTask = (() => {
    if (typeof MessageChannel !== "function") return () => Promise.resolve();
    const queue = []; const channel = new MessageChannel();
    channel.port1.onmessage = () => { const resolve = queue.shift(); resolve?.(); };
  ...
  function yieldToBrowser() {
    ...
    return new Promise(resolve => {
      __rmlWorkerYieldTask().then(resolve);
    });
  }
