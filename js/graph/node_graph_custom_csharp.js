...
async function openCustomCSharpFileGraphReady(
    ...
    if (signal?.aborted) {
      throw signal.reason ||
        new DOMException(
          window.RMLI18n.t("ui.literal.8e9f63c64593"),
          "AbortError"
        );
    }
    let settled = false;
    ...
    setCustomCSharpSynchronizationStatus(
      normalizedFileNodeId,
      window.RMLI18n.t("ui.literal.d26a14d1e2a1")
    );
    let opened = false;
    try {
...
function closeCustomCSharpFileGraph({
    ...
    const workSession = commit
      ? beginGraphTransitionWork({
          ...
          message:
            window.RMLI18n.t("ui.auto.4b1a8444d281"),
          detail:
            window.RMLI18n.t("ui.literal.75341d022e9c"),
          progress: 46
        })
      : 0;
...
function setCustomCSharpLiveDiagnosticSnapshot(
    ...
    const output = !logOutput ? [] : normalized.length
      ...
      : [{ source: "Roslyn", tone: "success", message: window.RMLI18n.t("ui.auto.5722d4c193db") }];
...
function applyCustomCSharpSynchronizationControl(
    ...
    const label = synchronizing
      ? openingAfterSynchronization
        ? window.RMLI18n.t("js.presentation.2b495039d30e")
        : window.RMLI18n.t("ui.literal.0e07427689f6")
      : needsOptimization
        ? window.RMLI18n.t("ui.literal.392d681e8615")
        : window.RMLI18n.t("ui.literal.3bd6b1342ed6");
    const actionId = synchronizing
      ? "synchronizing-node-graph"
      : needsOptimization
        ? "optimize-open-node-graph"
        : "open-node-graph";
    if (button.getAttribute("aria-label") !== label || button.dataset.actionId !== actionId) {
      setInspectorButtonContent(button, label, actionId);
    }
    button.setAttribute(
      "aria-label",
      label
...
function cancelCustomCSharpSynchronization(
    ...
    const error = new DOMException(
      window.RMLI18n.t("ui.literal.90e2466e1463"),
      "AbortError"
    );
...
function customCSharpWorkerTransport() {
    ...
    if (
      ...
      throw new Error(
        window.RMLI18n.t("ui.literal.cf57b88b0c7a")
      );
    }
    return transport;
  }
...
function buildCustomCSharpFragmentInWorker(nodeId, source, parseResult, options) {
    if (typeof Worker !== "function") {
      return Promise.reject(
        new Error(
          window.RMLI18n.t("ui.literal.d678e23544d6")
        )
      );
    }
    let transport;
    ...
    if (previous) {
      previous.abort(
        new DOMException(
          window.RMLI18n.t("ui.literal.7da0651ee800"),
          "AbortError"
        )
      );
    }
    const worker = new Worker(
      new URL(
        "js/workers/graph_codegen_worker.js?v=1.21.03-universal-presentation-dev400-edit-mode-sidebar-access",
        document.baseURI
      ),
    ...
    return new Promise((resolve, reject) => {
      ...
      const fail = (
        ...
        const message =
          diagnostics[0] ||
          window.RMLI18n.t("ui.literal.6cba60c04d9f");
      ...
      worker.addEventListener("message", event => {
        ...
        if (response.progress === true) {
          setCustomCSharpSynchronizationStatus(
            ...
            String(
              response.message ||
                window.RMLI18n.t("ui.literal.7c498a969008")
            ),
            { source: "Worker" }
          );
          return;
      ...
      worker.addEventListener("error", event => {
        const message =
          event.message ||
          window.RMLI18n.t("ui.literal.68fb750415a9");
        const location = [
          event.filename,
      ...
      worker.addEventListener(
        ...
        () => {
          fail({
            name: "DataCloneError",
            message:
              window.RMLI18n.t("ui.auto.a342a6402aad")
          });
        }
      );
      void (async () => {
...
async function openCustomCSharpFileGraphSynced(nodeId, options = {}) {
    ...
    const workSession = openAfterSync
      ? beginGraphTransitionWork({
          kicker: window.RMLI18n.t("ui.text.ba090b5e07cf"),
          title: window.RMLI18n.t("ui.auto.56d201fa0b9c"),
          message:
            window.RMLI18n.t("ui.auto.6554e2ad75b4"),
          detail:
            window.RMLI18n.t("ui.literal.89c5836716cb"),
          progress: 18
        })
      : 0;
    try {
    ...
    setCustomCSharpSynchronizationStatus(
      normalizedNodeId,
      window.RMLI18n.t("ui.literal.f2b91ab15b92"),
      { source: "Builder" }
    );
    const task =
...
async function synchronizeCustomCSharpFileGraph(
    ...
    const assertCurrentProject = () => {
      ...
      if (
        ...
        throw new DOMException(
          window.RMLI18n.t("ui.literal.e33a5f6e3199"),
          "AbortError"
        );
      }
    };
    ...
    const assertCurrentOwnerContext = () => {
      ...
      if (
        ...
        throw new DOMException(
          window.RMLI18n.t("ui.literal.e044f5fcd809"),
          "AbortError"
        );
      }
    };
    ...
    setCustomCSharpSynchronizationStatus(
      ownerId,
      window.RMLI18n.t("ui.literal.c2c6ee4d187c")
    );
    try {
      ...
      if (String(currentOwner?.parameters?.source || "") !== source) {
        if (!quiet) showGraphMessage(window.RMLI18n.t("ui.literal.76085d9b0828"), "warning");
        return false;
      }
      ...
      if (
        ...
        setCustomCSharpSynchronizationStatus(
          ownerId,
          window.RMLI18n.t("ui.literal.cb339d4c5095")
        );
        ...
        if (
          ...
          if (!roundtripInputStillCurrent) {
            if (!quiet) {
              showGraphMessage(
                window.RMLI18n.t("ui.literal.90e6bffa941c"),
                "warning"
              );
            }
            return false;
          ...
          if (
            ...
            setCustomCSharpSynchronizationStatus(
              ownerId,
              window.RMLI18n.t("ui.literal.330b8487594e")
            );
      ...
      const validatePreparedGraph = async candidate => {
        setCustomCSharpSynchronizationStatus(
          ownerId,
          window.RMLI18n.t("ui.literal.d9aeebf11c28")
        );
        ...
        if (rendered?.ok !== true) {
          const messages =
            ...
            rendered.diagnostics.length > 0
              ...
              : [window.RMLI18n.t("ui.literal.697236a2863e")];
        ...
        if (validation?.ok !== true) {
          ...
          preparedGraphValidationFailure =
            messages[0] ||
            window.RMLI18n.t("ui.literal.2a4eaa325753");
          setCustomCSharpDiagnostics(
            ownerId,
            messages.length > 0
              ? messages
        ...
        preparedGraphValidationFailure = matches
          ? ""
          : window.RMLI18n.t("ui.literal.490c7a84f221");
        if (!matches) {
          setCustomCSharpDiagnostics(
            ownerId,
            [preparedGraphValidationFailure],
      ...
      if (!prepared?.ok || !await validatePreparedGraph(prepared)) {
        ...
        if (!fragment?.ok) {
          throw new Error(
            fragment?.diagnostics?.[0] ||
              window.RMLI18n.t("ui.literal.a09cadf2c243")
          );
        }
        prepared =
      ...
      setCustomCSharpSynchronizationStatus(
        ownerId,
        window.RMLI18n.t("ui.literal.bd722de9de3f")
      );
      const opened = await openCustomCSharpFileGraphReady(
        ownerId,
        signal
      ...
      new Promise((resolve, reject) => {
        ...
        script.addEventListener(
          ...
          () => {
            ...
            if (
              ...
              reject(
                new Error(
                  window.RMLI18n.t("ui.literal.157b4bf749a6")
                )
              );
            }
          },
        ...
        script.addEventListener(
          ...
          () => {
            reject(
              new Error(
                window.RMLI18n.t("ui.literal.0b2760f8fe2c")
              )
            );
          },
          { once: true }
...
function markCustomCSharpGraphAuthoritative(
    ...
    if (
      ...
      running.abort(
        new DOMException(
          window.RMLI18n.t("ui.literal.39bbbd9798ce"),
          "AbortError"
        )
      );
    }
...
function createCustomCSharpInlineFrame(
    ...
    if (dom.canvasTitle) {
      dom.canvasTitle.innerHTML =
        "<small>{{i18n:ui.text.ba090b5e07cf}}</small> {{i18n:ui.hardgate.embedded_code_editor}}";
    }
...
function createCustomCSharpOverlayFrame(
    ...
    const windowIcon = name =>
      `<svg viewBox="0 0 24 24" aria-hidden="true"><use href="assets/rml-icons.svg?v=1.21.03-universal-presentation-dev400-edit-mode-sidebar-access#icon-${name}"></use></svg>`;
...
function restoreGraphAfterCustomCSharpInlineEditor(
    ...
    const workSession =
      beginGraphTransitionWork({
        ...
        detail:
          window.RMLI18n.t("ui.literal.a7711a55a69b"),
        progress: 38
      });
    if (dom.canvasTitle) {
      dom.canvasTitle.innerHTML =
...
function prepareCustomCSharpEditorHost(
    ...
    stylesheet.href = new URL(
      "styles/features/styles.runtime-graph.css?v=1.21.03-universal-presentation-dev400-edit-mode-sidebar-access",
...
async function createCustomCSharpExternalHost(
    ...
    if (!hostWindow) {
      showGraphMessage(
        window.RMLI18n.t("ui.literal.a5694eeaa1f5"),
        "error"
      );
      return null;
    }
...
function mountCustomCSharpEditorPresentation({
    ...
    void loadCustomCSharpDetachedEditorModule()
      .then(async editorModule => {
        ...
        if (!node) {
          throw new Error(
            window.RMLI18n.t("ui.literal.747c640efbeb")
          );
        }
        let record;
        ...
        if (!mounted) {
          throw new Error(
            window.RMLI18n.t("ui.literal.64916f967909")
          );
        }
