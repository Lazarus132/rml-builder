(() => {
  ...
  function makeSetting(valueType, fieldName, keyName, defaultValue) {
    return {
      ...
      fieldName,
      keyName,
      description: window.RMLI18n.t("ui.auto.368018b4fa0f"),
  ...
  function normalizeControl(node) {
    ...
    node.hidden = true;
    node.description ||= window.RMLI18n.t("ui.literal.5b10c8d4c292");
  ...
  function requestRefresh(options = {}) {
    ...
    if (!graphViewActive()) {
      try { window.renderOutline?.(); } catch {}
      if (refreshInspector) {
  ...
  function renderDynamicInspector(force = false) {
        ...
        if (runtimeAvailable) {
          ...
          defaultNote.textContent =
            node.dynamicAllowEmpty
              ? window.RMLI18n.t("ui.literal.1b7dbd9f8006")
              : window.RMLI18n.t("ui.literal.b1469b5edffb");
    ...
    outputs.innerHTML = "<strong>{{i18n:ui.text.515bfbbc0ea4}}</strong><br>" +
      [node, ...companions]
  ...
  function ensureRuntimeBridgeSubscription() {
    ...
    try {
      ...
      console.warn(
        window.RMLI18n.t("ui.literal.2cec0f712b1d"),
        error
      );
  ...
  function dynamicPreviewChoiceItems(
    ...
    if (node.dynamicAllowEmpty === true) {
      items.push({
        label: window.RMLI18n.t("ui.auto.7c5fc7a0cb48"),
        value: ""
      });
    }
  ...
  function appendDynamicPreview() {
    ...
    for (const node of controls) {
      ...
      if (
        ...
        previousButton.innerHTML = `<svg class="rml-inline-icon" viewBox="0 0 24 24" aria-hidden="true"><use href="assets/rml-icons.svg?v=1.21.15-universal-presentation-dev414-export-compiled-copy-state-dedupe#icon-triangle-left"></use></svg>`;
        ...
        nextButton.className =
          "rml-preview-control rml-preview-enum-step";
        nextButton.innerHTML = `<svg class="rml-inline-icon" viewBox="0 0 24 24" aria-hidden="true"><use href="assets/rml-icons.svg?v=1.21.15-universal-presentation-dev414-export-compiled-copy-state-dedupe#icon-triangle-right"></use></svg>`;
  ...
  function dynamicProviderSource() {
    ...
    return globalThis.RMLCodeTemplates.text("configuration", "Dynamic_Settings_provider", [entries.join(",\n            "),
ownerClass,
csString(graphClass)]);
  }
  ...
  function wrapUi() {
    if (typeof window.renderPreview === "function" &&
        !window.renderPreview.__rmlDynamicWrapped) {
      const basePreview = window.renderPreview;
      ...
      window.renderPreview = wrappedPreview;
    }
...
(() => {
  ...
  function editableMakeSetting(
    ...
    return {
      ...
      keyName,
      description:
        window.RMLI18n.t("ui.auto.368018b4fa0f"),
      defaultValue:
        String(defaultValue ?? ""),
      hidden: true,
      validatorMode: "none",
  ...
  function createBoundControlRecord(source) {
    ...
    control.description =
      window.RMLI18n.t("ui.auto.c563abbaf207");
    control.reaction =
      "startup-saved";
  ...
  function installEditablePalette() {
    ...
    for (const entry of desired) {
      ...
      plus.innerHTML = `<svg class="rml-inline-icon" viewBox="0 0 24 24" aria-hidden="true"><use href="assets/rml-icons.svg?v=1.21.15-universal-presentation-dev414-export-compiled-copy-state-dedupe#icon-add"></use></svg>`;
